...
{
    public class ConsoleUpgradeSingleStep : IAppCommand
    {
        ...
        private readonly IOptions<SingleStepOptions> _singleStepOptions;

        public ConsoleUpgradeSingleStep(
            IUserInput input,
            ...
            IOptions<SingleStepOptions> singleStepOptions)
        {
            ...
            _singleStepOptions = singleStepOptions ?? throw new ArgumentNullException(nameof(singleStepOptions));
        }
        ...
        public async Task RunAsync(CancellationToken token)
        {
            ...
            try
            {
                ...
                if (step is not null)
                {
                    if (_singleStepOptions.Value.Step is null || step.Id == _singleStepOptions.Value.Step)
                    {
                    ...
                    {
                        _logger.LogError("Step did not match. Next step is: " + step.Id);
                        return;
                    }
                }

                _logger.LogInformation("Upgrade step has completed. Please review any changes.");
            }
            finally
            {
